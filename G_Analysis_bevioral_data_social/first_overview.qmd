...
problem_data <- data_phil %>% 
  ...
  mutate(n_explore_after_gem = length(unique(cell)),
         remaining_trials = 25-gem_found,
         proportion_explore = n_explore_after_gem/remaining_trials) 

problem_data%>% 
  select(unique_rounds,proportion_explore) %>% 
  distinct() %>% 
  ggplot()+
  geom_histogram(aes(x = proportion_explore),binwidth = .02 ) +
  theme_bw(base_size = 20) +
  labs(title = "behavior after gem was found")
...
all_data %>% 
  filter(gempresent==1) %>% 
  ggplot() +
  geom_boxplot(aes(x = social_info_factor, y = round_gem_found)) 
