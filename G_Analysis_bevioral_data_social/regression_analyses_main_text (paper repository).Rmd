...
points_age_advisor_model <-
  brms::brm(formula = scaled_points ~ demo_quality_f * age_f + (1 |  uniqueID),
            prior = prior_cauchy,
            sample_prior = TRUE,
            data = data_regression_points,
            iter = 6000,
            chains = 6)
## save
save("points_age_advisor_model",
     file = paste0(here(),'/G_Analysis_bevioral_data_social/modelfits/points_age_advisor_model.RData'))

  }
#
# ## results
tab_model(points_age_advisor_model)
plot_model(points_age_advisor_model)


brms::conditional_effects(points_age_advisor_model)
...
## When do participants find gems?
...
tab_model(gem_age_advisor_model)
