...
all_data %>% 
  ...
  distinct() %>% 
  ungroup() %>% 
  t.test(mean_points_player ~ age_f, data = ., alternative = "two.sided", paired = FALSE)
...
## results model 2
tab_model(performance_age_demonstrator_model_scaled)plot_model(performance_age_demonstrator_model_scaled)
