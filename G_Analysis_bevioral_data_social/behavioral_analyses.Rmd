...
```{r}
  ...
  all_data %>%
  filter(gempresent == 1 &gem_found_how=='copier' )%>%
  ungroup %>%
...
```{r differences in social info use}
all_data %>% group_by(demo_type, uniqueID, round, group, gem_found, age, demo_quality) %>%
...
poisson_reg_copy_bayseian <- 
all_data %>%
  group_by(demo_type, uniqueID, round, group, gem_found, age, demo_quality) %>%
  filter(social_info_use == "copy" & gem_found_how != 'alone') %>%
  count(social_info_use) %>% 
  brms::brm(n ~ demo_quality * group + (1 | uniqueID), data = ., family = poisson())

summary(poisson_reg_copy_bayseian)


tab_model(poisson_reg_copy_bayseian)
plot_model(poisson_reg_copy_bayseian, axis.lim = c(.2,2)) +
