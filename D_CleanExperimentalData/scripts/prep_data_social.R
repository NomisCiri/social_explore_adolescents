...
data <- data %>% 
  ...
  mutate(performance_group_f = ifelse(
    ...
    ifelse(performance_group == 2, "medium",
           "high"))) %>% 
    ungroup() %>% 
    mutate(social_info_type = case_when( soc_info_round %in% c(800,921,254,1599,1899,408,838) ~ 1,
                           soc_info_round %in% c(1650, 504, 376, 868, 332, 1434, 1177, 1244, 468, 1639) ~ 2,
                           soc_info_round %in% c(905, 1625, 1912, 335, 343, 795) ~ 3,
                           TRUE ~ NaN ),
           social_info_factor = ifelse(social_info_type == 1, "no gem", ifelse(social_info_type==2, "gem not found", ifelse(social_info_type == 3, "gem found", NA)))
)
