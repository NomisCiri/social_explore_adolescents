...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("rounds"
                  , label = "select_round",
                  choices = unique(explore_data$unique_rounds)),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$selected_round,{
    ...
    output$plot <-  renderPlotly({
      ...
      ggplotly(make_plot(dat = plotData() )) %>%
        animation_opts(transition = 0)
    })
  })
  ...
  observeEvent(input$selected_round,{
    ...
    output$min_max <- renderText({
      ...
      paste(
        as.numeric(input$selected_round))
    })
  ...
  outputOptions(output, "unique_round", suspendWhenHidden = FALSE)  
