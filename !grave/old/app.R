...
server <- function(input, output) {
  ...
  gpr <- function(X.test, theta, X, Y, k) {
    ...
    result <- apply(Xstar, 1, function(x) {
      ...
      if (cv < 0) {
        cv <- abs(cv)
      } # TEMPORARY SOLUTION: MANUALLY SET CV TO BE POSITIVE IF NEGATIVE
  ...
  output$density <- renderPlot({
    ...
    SD <- EnvirionemntAdol %>%
      dplyr::rowwise() %>%
      mutate(value = rnorm(1, mean = Mean, sd = Sigma)) %>%
      ungroup() %>%
    ...
    for (nround in 2:2) {
      ...
      dummy <- data.frame(
        trial = 0:Iteration, x = as.numeric(X[, 1]), y = as.numeric(X[, 2]),
    ...
    dummydat %>% ggplot(aes(x = x, y = y, fill = density)) +
      ...
      ylab("SD") +
      xlab("Mean") +
      ggtitle(paste0("Sampling Density ", trials)) +
      theme_cowplot()
  }) # endrounds
}
