...
environments_with_gems<-gem_envs%>%do.call(rbind,.)%>%dplyr::rowwise()%>%mutate(value=rnorm(1,mean = Mean,sd=Variance))%>%
  ...
  theme_minimal(14)+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),aspect.ratio = 1)
ggsave(environments_with_gems,filename = "environments_with_gem.png")

environments_without_gems<-no_gem_envs%>%do.call(rbind,.)%>%dplyr::rowwise()%>%mutate(value=rnorm(1,mean = Mean,sd=Variance))%>%
  ggplot(aes(x=x,y=y,fill=value))+geom_tile()+
  scale_fill_continuous_divergingx(palette = 'Spectral', mid = 0)+
