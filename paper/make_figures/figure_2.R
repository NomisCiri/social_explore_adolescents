...
## load packages
pacman::p_load(tidyverse, gghalves, here, lmerTest, ggthemes, cowplot, ggh4x)

## load data
all_data <- read_csv(file = paste0(here(), "/data/social/data_social_all_participants_08-2024.csv"))

labels_a <- c(
  `worst` = "Low: Explores until the end",
  `medium` =  "Medium: Exploits a non-gem",
  `best` = "High: Finds a gem")
...
#color = "black",
 ...
 # scale_x_discrete(labels = c("High (Gem)", "Medium", "Low")) +
  ...
  facet_wrap(~ demo_quality_f, labeller = as_labeller(labels_a), scales = "free_x") +
  theme_base(15) +
  theme(#legend.position = "none" ,
        plot.background = element_blank(),
...
## save figure
ggsave("plots/figure2.png", figure2, height = 7, width = 10, scale = 1)
