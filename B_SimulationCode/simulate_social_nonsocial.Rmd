...
social_weights=seq(from=0,to=2,length.out=20)
#otherLoc=0
      ...
      for (soc_w in social_weights){
        ...
        explorations=rbind(explorations,
                           explore_env_social(explore_func=bayesianMeanTracker,
                                             ...
                                             cntrl=cntrl_social,
                                             rep)%>%
                             mutate(soc_w=soc_w,demonstrator=oth,repetition=rep,env=indx)
        )
    }
  }
  explorations
}
saveRDS(file="../A_GeneratedFiles/simulations/soc_sims_selected_demonstrator",object=Plot_dat_social_All)
# stopCluster(cl)

} else{
  Plot_dat_social_AllNew<-readRDS(file="../A_GeneratedFiles/soc_sims_selected_demonstrator")
}
```
