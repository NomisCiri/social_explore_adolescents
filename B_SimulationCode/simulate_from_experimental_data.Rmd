...
modelSim <-
  function(par,
    ...
    choices_this_round <- data.frame(
      round = r,
      choice = indices,
      env_number = env_numbers[counter],
      rewards = numeric(24)
    )
    
    ## ADD ASSOCIATED REWARDS OF THESE CHOICES
    this_env <- envs[env_numbers[counter]]
    
    for (row in 1:nrow(choices_this_round)){
      
      
    #browser()
      choices_this_round$rewards[row] <- rnorm(1, this_env[[1]]$Mean[choices_this_round$choice[row]], this_env[[1]]$Variance[[choices_this_round$choice[row]]])
    }

     choices <- rbind(choices, choices_this_round)          
