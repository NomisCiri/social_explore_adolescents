...
load_envs_social <- function(path = "A_GeneratedFiles/") {
  ...
  envs_no_gems_list <-
    rjson::fromJSON(file = paste0(path, "experiment/env_no_gems.json")) # max gem value: 250; variance: 25
  
  envs_gems_list <-
    rjson::fromJSON(file = paste0(path, "experiment/env_gems.json")) # max gem value: 250; variance: 25
