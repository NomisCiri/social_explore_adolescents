...
# load packages
pacman::p_load(tidyverse, rjson, DEoptim, doParallel, jsonlite, data.table)
  ...
  function(par,
    ...
    for (r in unique(trainingSet$round)) {
      ...
      for (i in 1:(horizon - 1)) {
        ...
        choices <- data.frame(choice_index = x1 + 1 + (x2 * 8),
...
foreach(
  ...
  model_fit <-
    as.data.frame(readRDS(paste0("A_GeneratedFiles/modelfits/all_envs/GM_fit_", playerNr, ".rds"))) %>% 
  ...
  saveRDS(simulated_choices,
          file = paste0("A_GeneratedFiles/simulations/all_envs/simulated_choices", playerNr))
