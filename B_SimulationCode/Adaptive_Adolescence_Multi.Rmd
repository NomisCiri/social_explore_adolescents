...
```{r}
#envirionmentMeanAdol=seq(-100,100,length.out=8)
#envirionmentVarianceAdol=seq(1,80,length.out=8)
#EnvirionemntAdol1=expand.grid(Mean=envirionmentMeanAdol,Variance=envirionmentVarianceAdol)

envirionmentMeanAdol=seq(-100,100,length.out=10)
envirionmentVarianceAdol=seq(1,80,length.out=10)
dat_social=expand.grid(x1=1:10,x2=1:10)
...
for (i in gems){
  ...
  index=sample(x=1:length(EnvirionemntAdol[,1]),size=i)
  Gem=rnorm(i,mean=150,sd=10)
  EnvirionemntAdol[index,1]=Gem
  GemEnvs[[i]]=EnvirionemntAdol%>%mutate(HowManyGems=i)%>%mutate(x=dat_social$x1,y=dat_social$x2)
}
EnvirionemntAdol=expand.grid(Mean=envirionmentMeanAdol,Variance=envirionmentVarianceAdol)
GemEnvs[[i+1]]=EnvirionemntAdol%>%mutate(HowManyGems=0)%>%mutate(x=dat_social$x1,y=dat_social$x2)# note: 0 gems is always last!!!

library(colorspace)
GemEnvs%>%do.call(rbind,.)%>%dplyr::rowwise()%>%mutate(value=rnorm(1,mean = Mean,sd=Variance))%>%
  ...
  facet_wrap(~HowManyGems,scales="free")+
...
if (loadfromdisk==F){
  ...
  saveRDS(file="../A_GeneratedFiles/Plot_dat_social_All_04_4_22.rds",object=Plot_dat_social_All)
  # stopCluster(cl)
  
} else{
  Plot_dat_social_All<-readRDS(file="../A_GeneratedFiles/Plot_dat_social_All.rds")
  Plot_dat_social_AllNew<-readRDS(file="../A_GeneratedFiles/Plot_dat_social_All_04_4_22.rds")
...
ggsave(environments_without_gems,filename = "environments_without_gem.png")
...
if (loadfromdisk==F){
  ...
  saveRDS(file="../A_GeneratedFiles/Plot_dat_social_AllProb_07_6_22.rds",object=Plot_dat_social_AllProb)
  # stopCluster(cl)
} else{
  Plot_dat_social_All<-readRDS(file="../A_GeneratedFiles/Plot_dat_social_All.rds")
  Plot_dat_social_AllNew<-readRDS(file="../A_GeneratedFiles/Plot_dat_social_AllProb_07_6_22.rds")
...
ah<-matrix(1:64,nrow = 8,ncol = 8)#%>%as.array()
rotate <- function(x) t(apply(x, 2, rev))
rotate(ah)

order=ah%>%rotate()%>%as.numeric()
no_gem_envs[[1]][order,]

no_rot<-ggplot(no_gem_envs[[1]],aes(x=x,y=y,fill=Mean))+geom_tile()+
  scale_fill_continuous_divergingx(palette = 'Spectral', mid = 0)
