...
pacman::p_load(tidyverse,rjson,DEoptim)
...
cvfun<-function(selector, kernelFun, acquisition, leaveoutindex){
  ...
  d1<-subset(explore_data, player==selector & gempresent == 1)
  #training set
  rounds <- 1:6
...
for (r in 1:6){ #loop through rounds in roundList
  ...
  cv <- cvfun(selector=1, kernelFun=bayesianMeanTracker, acquisition = ucb, leaveoutindex=r)# only try one sub
