...
cvfun<-function(selector, kernelFun, acquisition, leaveoutindex){
  ...
  d1<-subset(explore_data, player==selector & gempresent == 0)
  #training set
  rounds <- 1:6
...
foreach(
  ...
  print(x)
  for (r in 1:6){ #loop through rounds in roundList
    
    cv <- cvfun(selector=x, kernelFun=bayesianMeanTracker, acquisition = ucb, leaveoutindex=r)# only try one sub
    routput <- rbind(output, cv)
  }
  saveRDS(routput,file=paste0("A_GeneratedFiles/modelfits/fit_",x))
}
