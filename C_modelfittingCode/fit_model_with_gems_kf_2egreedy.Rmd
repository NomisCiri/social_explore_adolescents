...
social_data%>%group_by(demo_type,player,round,group)%>%
  count(social_info_use)%>%filter(!is.na(social_info_use))%>%
...
social_data%>%group_by(demo_type,player,round,group)%>%
  ...
  brms::brm(n~group*demo_type,data=.,family="poisson",chains=4,cores=4)
```
# Fit model
...
if (refit){
  ...
  #  for (r in rounds) { # loop through rounds in roundList
      cv_one<-fit_bmt_ucb_softmax_egreedy(d1 = d1,leaveoutindex = NA)
...
social_fits%>%select(beta,group,player)%>%unique()%>%
  lm(beta~group,data = .)%>%anova()
...
social_fits%>%select(epsilon_gem,group,player)%>%unique()%>%
  lm(epsilon_gem~group,data = .)%>%anova()
...
social_fits%>%select(tau,group,player)%>%unique()%>%
lm(tau~group,data = .)%>%anova()
