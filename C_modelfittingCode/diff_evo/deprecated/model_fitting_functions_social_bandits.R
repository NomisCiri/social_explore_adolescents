...
fit_3sw <- function(d1, rounds) {
  ...
  fit <- DEoptim(
    ...
    upper = ubound, 
    dat = d1#,
    #  DEoptim.control(itermax = 10,NP=8)
  )
  paramEstimates <- fit$optim$bestmem # MODEL DEPENDENT PARAMETER ESTIMATES
...
fit_slr<- function(d1, rounds) {
  # Set upper and lower bounds based on nParams
  lbound <- c(0,0,0,0,-10) 
  ubound <- c(20,0.2,1,1,1)                           
