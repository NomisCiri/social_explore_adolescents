...
if (refit){
  ...
  social_fits <- foreach(
    ...
    for (r in rounds) { # loop through rounds in roundList
      cv_one<-fit_bmt_ucb_slr_softmax_egreedy(d1 = d1,leaveoutindex = r)
    ...
    saveRDS(cv, file = paste0("./A_GeneratedFiles/modelfits/social_models/all_envs/kalman_ucb_slr_softmax_egreedy/fit", player_nr, ".rds"))
    saveRDS(d1, file = paste0("./A_GeneratedFiles/modelfits/social_models/all_envs/kalman_ucb_slr_softmax_egreedy/data_fit", player_nr, ".rds"))
    return(d1)
  }
} else{#read from disk if refit is false
   fits_path<-"A_GeneratedFiles/modelfits/social_models/all_envs/kalman_ucb_slr_softmax_egreedy/"
