...
source(here("./C_modelfittingCode/","diff_evo/","models_mle.R"))

## load functions that initialize the model fit
source(here("./C_modelfittingCode/","diff_evo/","model_fitting_functions_social_bandits.R"))
...
if (fitagain){
  ...
  social_fits <-foreach(
    ...
    fit=T
    if(length(d1$player) == 25 & fit){
      cv <- fit_sw(d1 = d1) # only try one sub
      ...
      #if you did not see this demonstrator, make null. 
      return(NULL)
    }
  }
...
```{r}
social_fits %>% filter(soc_info_round!=795)%>%
  dplyr::select(uniqueID, group, sw_1,soc_info_round,demo_quality) %>% 
  ggplot(aes(x = as.factor(soc_info_round), y = sw_1, color = demo_quality,shape=group)) +
  geom_point(alpha=0.001,position=position_dodge(0.9))+
  stat_summary() +
  #stat_boxplot()+
  theme_minimal()
...
social_fits %>%
  dplyr::select(uniqueID, group, tau_1,soc_info_round,demo_quality) %>% 
  ggplot(aes(x = demo_quality, y = tau_1, color = demo_quality,shape=group)) +
  geom_point(alpha=0.01,position=position_dodge(0.9))+
  stat_summary() +
  #stat_boxplot()+
  theme_minimal()
...
social_fits %>%
  dplyr::select(uniqueID, group, lr_p,soc_info_round,demo_quality) %>% 
  ggplot(aes(x = demo_quality, y = lr_p, color = demo_quality,shape=group)) +
  geom_point(alpha=0.01,position=position_dodge(0.9))+
  stat_summary() +
  #stat_boxplot()+
  theme_minimal()
...
what_is_this%>%ggplot(aes(x=trial,y =points, color = group)) +
  ...
  facet_grid(demo_quality~gemlabel)+
  theme_bw()
...
Did adolescents use SI most beneficially?
