...
#
social_data%>%select(player,group)%>%unique()
```
...
if (refit){
  ...
  social_fits <- foreach(
    ...
    #for (r in rounds) { # loop through rounds in roundList
      cv_one<-fit_2lr_4sw_softmax_egreedy(d1 = d1,leaveoutindex = NA)
    ...
    saveRDS(cv, file = paste0("./A_GeneratedFiles/modelfits/social_models/all_envs/utility_2lr_4sw_softmax_egreedy/fit", player_nr, ".rds"))
    saveRDS(d1, file = paste0("./A_GeneratedFiles/modelfits/social_models/all_envs/utility_2lr_4sw_softmax_egreedy/data_fit", player_nr, ".rds"))
    return(d1)
  }
} else{#read from disk if refit is false
  fits_path<-"A_GeneratedFiles/modelfits/social_models/all_envs/utility_2lr_4sw_softmax_egreedy/"
...
for (player_nr in unique(social_fits_dNa$uniqueID)) {
  ...
  ####
  #for (r in rounds) { # loop through rounds in roundList
  cv <- simumalte_2lr_3sw_softmax_egreedy(
    par = estimates,
