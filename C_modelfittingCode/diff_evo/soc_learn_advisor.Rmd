...
social_data <- social_data %>% group_by(uniqueID,round) %>%
  ...
  #  mutate(social_info<-ifelse(social_info==65,1,social_info))
  #filter(social_info<=64)
  filter(!Var) %>% select(-Var)%>%group_by(uniqueID)%>%ungroup()

#social_data%>%filter(Var==1)%>%select(social_info_use,cell,cells,social_info)->inspect
...
if (fitagain){
  ...
  saveRDS(social_fits,file = here("A_GeneratedFiles","modelfits","tau_slearn_rangs_oth_prior.rds"))
}else{
...
<!-- BIC_old<-social_fits_old%>%select(fit,uniqueID)%>% -->
<!--   unique()%>% -->
<!--   mutate(BIC=fit+2*log(12*25), -->
<!--          model="no_lr") -->

<!-- BIC_new<-social_fits%>%select(fit,uniqueID)%>% -->
<!--   unique()%>% -->
