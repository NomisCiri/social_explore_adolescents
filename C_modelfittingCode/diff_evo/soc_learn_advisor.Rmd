...
social_data <- social_data %>% group_by(uniqueID,round) %>%
  mutate(social_info_p1=social_info+1)%>%#group_by(round)
  mutate(Var=any(social_info_p1>64))%>%group_by(uniqueID)%>%ungroup()#%>%filter(Var!=1)#%>%select(social_info_use,cell,cells,social_info)#->inspect

#social_data

soc_inf<-unique(social_data$soc_info_round)
social_data<-social_data%>%filter(p_value_rand<0.01)

#length(social_data$p_value_rand[social_data$p_value_rand<0.00001])

social_data%>%mutate(qual=ifelse(env_number==6,1,qual))%>%filter(env_number!=7)%>%#filter(env_number==6)%>%
  ggplot(aes(x=group,y=points,color=qual))+
  stat_summary()+
  facet_wrap(. ~ env_number,scales = "free")


social_data%>%ggplot(aes(y=copy,x=demonstrator,color=as.factor(env_number)))+
  stat_summary()+
...
if (fitagain){
  social_fits <- foreach(
    ...
    cv <- fit_slr(d1 = d1,environments=environments) # only try one sub
  ...
  saveRDS(social_fits,file = here("A_GeneratedFiles","modelfits","tau_slearn_rangs_no_prior.rds"))
}else{
...
BIC_prior<-social_fits_old%>%select(fit,uniqueID)%>%
  unique()%>%
  mutate(BIC=fit+2*log(12*25),
         model="no_lr")

BIC_noprior<-social_fits%>%select(fit,uniqueID)%>%
  unique()%>%
  mutate(BIC=fit+4*log(12*25),
         model="lr")

rbind(BIC_new,BIC_old)%>%
  ggplot(aes(x=model,y=BIC))+
  stat_summary()

wilcox.test(BIC_old$BIC,BIC_new$BIC,paired=T)
...
parameter_plot<-plot_grid(alpha,omega,tau,labels="auto")


plot_df%>%filter(var_greek=="zeta")%>%
  ggplot(aes(x=value,color=group,group=group))+
  stat_density()+
...
alpha_mod<-social_fits %>% #filter(soc_info_round!=795)%>%
  ...
  #mutate(sw_1=(((sw_1-mean(sw_1))/sd(sw_1))*0.5))%>%
...
omega_mod<-social_fits %>% #filter(soc_info_round!=795)%>%
  ...
  mutate(lr=(((lr-mean(lr))/sd(lr))*0.5))%>%
...
tau_mod<-social_fits %>% #filter(soc_info_round!=795)%>%
  ...
  mutate(tau_1=(((tau_1-mean(tau_1))/sd(tau_1))*0.5))%>%
