...
if (refit){
  ...
  social_fits <- foreach(
    ...
    #  for (r in rounds) { # loop through rounds in roundList
      cv_one<-fit_bmt_social_egreedy_2ps(d1 = d1,leaveoutindex = NA)
   ...
   # }
    ...
    d1$epsilon_gem <- mean(cv[,7],na.rm=T)#%>%exp()
    d1$tau_gem <- mean(cv[,8],na.rm=T)#%>%exp()
    d1$soc_w_gem <- mean(cv[,9],na.rm=T)#%>%exp()
    return(d1)
  }
} else{
  #read from disk if refit is false
   social_fits_p<-read_csv(file = "A_GeneratedFiles/modelfits/social_models/kf_2p_greedy_social.csv")
}
...
social_fits_p<-social_fits%>%rowwise()%>%
  mutate(epsilon_p=exp(epsilon)/sum(exp(epsilon),exp(soc_w),exp(tau)),
         ...
         epsilon_gem_p=exp(epsilon_gem)/sum(exp(epsilon_gem),exp(soc_w_gem),exp(tau_gem)),
         tau_gem_p=exp(tau_gem)/sum(exp(epsilon_gem),exp(soc_w_gem),exp(tau_gem)),
         soc_w_gem_p=exp(soc_w_gem)/sum(exp(epsilon_gem),exp(soc_w_gem),exp(tau_gem))
         )

write_csv(social_fits_p,file = "A_GeneratedFiles/modelfits/social_models/kf_2p_greedy_social.csv")
```
...
social_fits_p %>%
  ...
  social_fits%>%select(error_var,group,player)%>%unique()%>%
...
social_fits_p %>%
  select(player, group, epsilon_gem_p) %>% 
  distinct() %>% 
  ggplot(aes(x = group, y = epsilon_gem_p, color = group)) +
  #geom_line(aes(group=index))+
  stat_boxplot()+
  theme_minimal()

social_fits_p%>%select(epsilon_gem_p,group,player)%>%unique()%>%
  lm(epsilon_gem_p~group,data = .)%>%anova()
...
# check greedy difference in kids and adults 
social_fits_p %>%
  dplyr::select(player, group, tau_gem_p) %>%
  distinct() %>%
  #mutate(asymmetry_index = (lr_p-lr_n)/(lr_p + lr_n)) %>%
  ggplot(aes(y = tau_gem_p, x=group, color = group)) +
  #geom_line(aes(group=index))+
  stat_boxplot()+
  theme_minimal()

social_fits_p%>%select(tau_gem_p,group,player)%>%unique()%>%
  lm(tau_gem_p~group,data = .)%>%anova()


# check greedy difference in kids and adults 
social_fits_p %>%
  dplyr::select(player, group, soc_w_gem_p) %>%
  distinct() %>%
  #mutate(asymmetry_index = (lr_p-lr_n)/(lr_p + lr_n)) %>%
  ggplot(aes(y = soc_w_gem_p, x=group, color = group)) +
  #geom_line(aes(group=index))+
  stat_boxplot()+
  theme_minimal()

#tau makes a difference
social_fits_p%>%select(soc_w_gem_p,group,player)%>%unique()%>%
lm(soc_w_gem_p~group,data = .)%>%anova()
