...
social_data <-  read_csv(file = paste0("./data/social/data_social_all_participants.csv")) %>%#drop_na()
  filter(tot_points>0 & !is.na(demo_type))%>%filter(player_nr<30)%>%# get a sub-sample of both adults and adolescents 
...
if (refit){
  ...
  social_fits <- foreach(
    ...
    for (r in rounds) { # loop through rounds in roundList
      cv_one<-fit_2lr_sw_softmax_egreedy(d1 = d1,leaveoutindex = r)
    ...
    saveRDS(cv, file = paste0("./A_GeneratedFiles/modelfits/social_models/all_envs/utility_2lr_sw_softmax_egreedy/fit", player_nr, ".rds"))
    saveRDS(d1, file = paste0("./A_GeneratedFiles/modelfits/social_models/all_envs/utility_2lr_sw_softmax_egreedy/data_fit", player_nr, ".rds"))
    return(d1)
  }
