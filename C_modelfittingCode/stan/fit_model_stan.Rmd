...
all_draws%>%group_by(participant,group,.variable,gem)%>%
  ...
  facet_grid(gem ~ .variable,scales="free_y",labeller = labeller(gem=c(`1`="Gem Found",`2`="Gem Not Found")))+
  theme_bw(14)->plot

plot


all_draws%>%group_by(participant,group,.variable,gem)%>%filter(.variable=="sw")%>%
  dplyr::summarise(m_value=mean(.value))%>%
  ggplot(aes(x=group,y=m_value,group=gem,color=as.factor(gem)))+
  #geom_point(position=position_dodge(0.9),alpha=0.05)+
  #geom_boxplot(position=position_dodge(0.1))+
  ...
  stat_summary()+
  #scale_color_discrete()+
  facet_grid(gem ~ .,labeller = labeller(gem=c(`1`="Gem Found",`2`="Gem Not Found")))+
  theme_bw(14)->plot_sw
