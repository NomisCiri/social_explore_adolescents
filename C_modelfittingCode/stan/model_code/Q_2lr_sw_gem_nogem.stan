...
transformed parameters{
  ...
  lr[,1]=Phi_approx(mus[1]+params_phi[,1])*2;// gains
  lr[,2]=Phi_approx(mus[2]+params_phi[,2])*2;// losses
  
  tau[,1]=exp(mus[3]+params_phi[,3]);
  tau[,2]=exp(mus[4]+params_phi[,4]);// after gem
  
  sw[,1]=Phi_approx(mus[5]+params_phi[,5])*30;// before gem
  sw[,2]=Phi_approx(mus[6]+params_phi[,6])*30;// before gem
  ...
  for (ppt in 1:N){  
    for (r in 1:R_subj[ppt]){
      ...
      for (t in 1:T_max){
        ...
        //update
        if(pe>=0){
          belief_means[choices[t,r,ppt]] +=  lr[ppt,1]*pe;
        }else{
          belief_means[choices[t,r,ppt]] +=  lr[ppt,2]*pe;
        }
...
generated quantities{
  ...
  // make logliks (simulations are more tricky because of the recursiveness and i cant define the environment in stan)
  for (ppt in 1:N){  
    for (r in 1:R_subj[ppt]){
      belief_means=rep_vector(0,64);
      ...
      for (t in 1:T_max){
        pe = rewards[t,r,ppt] - belief_means[choices[t,r,ppt]];
        //update
        belief_means[choices[t,r,ppt]] += pe>0 ? lr[ppt,1]*pe : lr[ppt,2]*pe;
