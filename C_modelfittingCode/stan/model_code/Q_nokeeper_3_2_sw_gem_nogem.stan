...
parameters {
  ...
  matrix[9,N] scale; // prior scaling for each parameter, add two additional ones for hyperprior of sw
...
transformed parameters{
  ...
  matrix<lower=0>[N,3] sw[2];//social weight
 // matrix[N, n_params] params_phi;// for non-centered paramatrezation
  ...
  sw[1,,1]=Phi_approx(mus[4]+sigmas[4] * scale[4,]')*30;
  sw[1,,2]=Phi_approx(mus[4]+sigmas[4] * scale[5,]')*30;
  sw[1,,3]=Phi_approx(mus[4]+sigmas[4] * scale[6,]')*30;
  
  sw[2,,1]=Phi_approx(mus[5]+sigmas[5] * scale[7,]')*30;
  sw[2,,2]=Phi_approx(mus[5]+sigmas[5] * scale[8,]')*30;
  sw[2,,3]=Phi_approx(mus[5]+sigmas[5] * scale[9,]')*30;
}
  ...
  for (ppt in 1:N){  
    for (r in 1:R_subj[ppt]){
      ...
      for (t in 1:T_max){
        ...
        belief_means_sw[social_info[t,r,ppt]] += sw[gem_found[t,r,ppt],ppt,demo_type[t,r,ppt]];
