...
if (refit){
 ...
 # source('./social_explore_adolescents/C_modelfittingCode/learning_models.R')
  ...
  social_fits <- foreach(
    ...
    cv <- fit_2lr_sw_gre(d1 = d1) # only try one sub
    ...
    d1$lr_p <- unlist(cv[2])
    d1$lr_n <- unlist(cv[3])
    d1$epsilon <- unlist(cv[4])
    d1$beta<-unlist(cv[5])
    d1$soc_w <- unlist(cv[6])
    
    #d1$prior<-unlist(cv[5])
    
    saveRDS(cv, file = paste0("../A_GeneratedFiles/modelfits/social_models/no_gems/Q2lr_sw_greedy/fit", player_nr, ".rds"))
    return(d1)
  }
} else{#read from disk if refit is false
  fits_path<-"A_GeneratedFiles/modelfits/social_models/no_gems/Q2lr_sw_greedy/"
...
social_fits %>%
  dplyr::select(player, group, epsilon) %>%
  distinct() %>%
  #mutate(asymmetry_index = (lr_p-lr_n)/(lr_p + lr_n)) %>%
  ggplot(aes(x = epsilon, fill = group)) +
  #geom_line(aes(group=index))+
...
social_fits %>%
  dplyr::select(player, group, beta) %>%
  distinct() %>%
  #mutate(asymmetry_index = (lr_p-lr_n)/(lr_p + lr_n)) %>%
  ggplot(aes(y = beta, x = group)) +
  geom_jitter(width = 0.01,alpha=0.1) +
  stat_summary()+
  #facet_wrap( ~ group) +
  theme_minimal()
