...
if (refit){
  ...
  #  for (r in rounds) { # loop through rounds in roundList
      cv_one<-fit_bmt_ucb_sw_softmax_egreedy(d1 = d1,leaveoutindex = NA)
   ...
   # }
    ...
    d1$soc_w <- mean(cv[,7],na.rm=T)

  
    return(d1)
  }
} else{#read from disk if refit is false
 social_fits<- read_csv(file = "A_GeneratedFiles/modelfits/kf_sw_softmax_greedy.csv")
}
write_csv(social_fits,file = "A_GeneratedFiles/modelfits/kf_sw_softmax_greedy.csv")
```
...
social_fits %>%
  ...
  lm(log(error_var)~group,data = .)%>%anova()
...
# check greedy difference in kids and adults 
social_fits %>%
  dplyr::select(player, group, soc_w,group) %>%
  distinct() %>%
  #mutate(asymmetry_index = (lr_p-lr_n)/(lr_p + lr_n)) %>%
  ggplot(aes(y = soc_w, x=group, color = group)) +
  #geom_line(aes(group=index))+
  stat_boxplot()+
  theme_minimal()

#tau makes a difference
social_fits%>%select(soc_w,group,player,age)%>%unique()%>%
lm(soc_w~group,data = .)%>%anova()
```
