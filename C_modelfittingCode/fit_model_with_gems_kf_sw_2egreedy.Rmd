...
social_data <-  read_csv(file = paste0("./data/social/data_social_all_participants.csv")) %>%#drop_na()
  filter(!is.na(demo_type) & round != 1)%>%#filter(player<30)%>%# get a sub-sample of both adults and adolescents 
...
if (refit){
  ...
  social_fits <- foreach(
   ...
   # for (r in rounds) { # loop through rounds in roundList
    ...
    saveRDS(cv, file = paste0("./A_GeneratedFiles/modelfits/social_models/all_envs/kalman_ucb_sw_softmax_egreedy/fit", player_nr, ".rds"))
    saveRDS(d1, file = paste0("./A_GeneratedFiles/modelfits/social_models/all_envs/kalman_ucb_sw_softmax_egreedy/data_fit", player_nr, ".rds"))
    return(d1)
  }
} else{#read from disk if refit is false
   fits_path<-"A_GeneratedFiles/modelfits/social_models/all_envs/kalman_ucb_sw_softmax_egreedy/"
...
#tau makes a difference
social_fits%>%select(soc_w,group,player)%>%unique()%>%
